...
type ProductCyclePlaceholderPageProps = {
  params: Promise<{
    productId: string;
  }>;
};

export default async function ProductCyclePlaceholderPage({
  params,
}: ProductCyclePlaceholderPageProps) {
  ...
  const title = detail.product
    ? `Ciclo articolo ${detail.product.code}`
    : "Ciclo articolo (placeholder)";

  return (
    <ConfigurationPlaceholder
      title={title}
      subtitle="Area read-only preparata per aggancio ciclo produttivo in wave successive."
